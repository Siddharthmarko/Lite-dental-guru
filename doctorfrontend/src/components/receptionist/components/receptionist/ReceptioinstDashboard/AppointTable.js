...
const AppointTable = () => {
  ...
  const fetchAppointments = async () => {
    try {
      const { data } = await axios.get(
        `https://huzaifdentalclinic.dentalguru.software/api/doctor/getAppointmentsWithPatientDetailsTreatSugg/${doctorId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleStatusChange = async (appointmentId, patient_uhid, newStatus) => {
    try {
      ...
      await axios.put(
        `https://huzaifdentalclinic.dentalguru.software/api/v1/receptionist/update-appointment-status`,
        {
          status: newStatus,
  ...
  const getTreatPackageData = async () => {
    try {
      const { data } = await axios.get(
        `https://huzaifdentalclinic.dentalguru.software/api/doctor/getTreatPackageViaTpidUhid/${branch}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleAction = async (action, appointId, uhid, appointment_status) => {
    ...
    try {
      ...
      await axios.put(
        `https://huzaifdentalclinic.dentalguru.software/api/doctor/upDateAppointmentStatus`,
        requestBody,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const res = await axios.get(
        `https://huzaifdentalclinic.dentalguru.software/api/doctor/appointtreatSitting?date=${selectedDate}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleStatusCancel = async (appointmentId, patient_uhid, newStatus) => {
    ...
    try {
      // Send a PUT request to your backend endpoint to update the status
      await axios.put(
        `https://huzaifdentalclinic.dentalguru.software/api/v1/receptionist/update-appointment-status-cancel`,
        {
          status: newStatus,
          cancelReason: reason,
          appointmentId: appointmentId,
                  ...
                  <tbody>
                    {currentRows?.map((patient, index) => {
                      const appointmentDate = new Date(
                        patient?.appointment_dateTime?.split("T")[0]
                      );
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/patient_profile/${patient.uhid}`}>
                              {patient.uhid}
                            </Link>
                          </td>
