...
const PrintSecurityAmt = () => {
  ...
  const getBill = async () => {
    try {
      const response = await axios.get(
        `https://laxmibhawanidentalhospital.dentalguru.software/api/v1/receptionist/getSinglePatientSecurityAmt/${branch}/${SId}`,
        {
          headers: {
  ...
  const getPatientTreatmentDetails = async (uhid) => {
    try {
      const response = await axios.get(
        `https://laxmibhawanidentalhospital.dentalguru.software/api/v1/receptionist/getPatientDeatilsByUhidFromSecurityAmt/${branch}/${uhid}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
