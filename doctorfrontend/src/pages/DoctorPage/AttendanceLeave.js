...
const AttendanceLeave = () => {
  ...
  const getAttendance = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getAttendancebyempId/${branch}/${employeeId}`,
        {
          headers: {
  ...
  const getLeaves = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/get-leaves/${branch}/${employeeId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
