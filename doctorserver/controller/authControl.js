...
const insertLab = (req, res) => {
  ...
  try {
    ...
    const selectQuery = `SELECT * FROM patient_lab_details WHERE tpid = ? AND test = ?`;
    db.query(selectQuery, [tpid, test], (err, result) => {
      if (err) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid tpid and test" });
      }
      if (result.length === 0) {
        ...
        db.query(sql, values, (err, result) => {
          if (err) {
            ...
            // logger.registrationLogger.log(
            //   "info",
            //   "Lab data inserted successfully"
            // );
            return res.status(201).json({
              success: true,
        ...
        return res
          .status(400)
          .json({ success: false, message: "Lab test already added" });
      }
    });
  } catch (error) {
